...
def get_reducers_type(ctx: FunctionContext) -> Type:
    ...
    * etc.
    ...
    if out_type.fullname not in {
        intoin_outtoout_out_fullname,
        chained_out_fullname,
    }:
        return ctx.default_return_type
    ...
    if out_type.fullname == chained_out_fullname:
        ...
        return Instance(
            ...
            [arg_types[0].args[0], arg_types[-1].args[1]],
        )
        ...
        return Instance(
            ctx.default_return_type.type,
            [arg_types[0].args[0], arg_types[0].args[1]],
        )
...
error_layer_input = ErrorCode(
    'layer-mismatch-input', 'Invalid layer input', 'Thinc'
)
error_layer_output = ErrorCode(
    'layer-mismatch-output', 'Invalid layer output', 'Thinc'
)
