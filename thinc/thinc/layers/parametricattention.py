...
def ParametricAttention(nO: Optional[int] = None) -> Model[InT, OutT]:
    ...
    return Model(
        'para-attn', forward, init=init, params={'Q': None}, dims={'nO': nO}
    )
...
def forward(
    ...
    attention, bp_attention = _get_attention(
        model.ops, Q, Xr.dataXd, Xr.lengths
    )
    output, bp_output = _apply_attention(
        model.ops, attention, Xr.dataXd, Xr.lengths
    )
...
def _get_attention(ops, Q, X, lengths):
    ...
    def get_attention_bwd(d_attention):
        d_attention = ops.backprop_softmax_sequences(
