...
def LSTM(
    ...
    *,
    ...
    if depth == 0:
        msg = (
            'LSTM depth must be at least 1. Maybe we should make this a noop?'
        )
...
def init(
    ...
    for i in range(depth):
        ...
        layer_nI = nH * dirs
    model.set_param(
        'LSTM', model.ops.xp.concatenate([p.ravel() for p in params])
    )
...
def _packed_to_padded(ops: Ops, Xr: Ragged, Xp: Padded) -> Padded:
    ...
    for t in range(Xp.size_at_t.shape[0]):
        ...
        start += batch_size
    return Padded(
        Y, size_at_t=Xp.size_at_t, lengths=Xp.lengths, indices=Xp.indices
    )
