...
SeqT = TypeVar(
    'SeqT', bound=Union[Padded, Ragged, List2d, Floats3d, PaddedData]
)
...
def forward(
    ...
    if isinstance(Xseq, Padded):
        ...
        return cast(
            Tuple[SeqT, Callable], _ragged_forward(layer, Xseq, is_train)
        )
        ...
        return cast(
            Tuple[SeqT, Callable],
            _array_forward(layer, cast(Floats3d, Xseq), is_train),
        )
    else:
        return cast(
            Tuple[SeqT, Callable],
            _list_forward(layer, cast(List2d, Xseq), is_train),
        )
...
def _is_padded_data(seq: SeqT) -> bool:
    return (
        isinstance(seq, tuple) and len(seq) == 4 and all(map(is_xp_array, seq))
    )
...
def _get_padded(model: Model, seq: SeqT) -> Padded:
    if isinstance(seq, Padded):
        ...
        size_at_t = model.ops.asarray1i(
            [floats3d_seq.shape[1]] * floats3d_seq.shape[0]
        )
        lengths = model.ops.asarray1i(
            [floats3d_seq.shape[0]] * floats3d_seq.shape[1]
        )
