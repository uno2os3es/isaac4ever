...
def with_ragged(layer: Model[Ragged, Ragged]) -> Model[SeqT, SeqT]:
    return Model(
        f'with_ragged({layer.name})', forward, init=init, layers=[layer]
    )
...
def forward(
    ...
    if isinstance(Xseq, Ragged):
        ...
        return cast(
            Tuple[SeqT, Callable], _padded_forward(layer, Xseq, is_train)
        )
        ...
        return cast(
            Tuple[SeqT, Callable],
            _list_forward(layer, cast(List, Xseq), is_train),
        )
...
def _padded_forward(
    ...
    Yr, get_dXr = layer(
        Ragged(flatten(Xs), layer.ops.asarray1i(lengths)), is_train
    )
