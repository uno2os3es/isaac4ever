...
class PyTorchShim(Shim):
    ...
    *not* a Thinc Model subclass itself.
    ...
    def __init__(
        ...
        deserialize_model: Optional[
            Callable[[Any, bytes, 'torch.device'], Any]
        ] = None,
    ):
    ...
    def predict(self, inputs: ArgsKwargs) -> Any:
        ...
        with torch.no_grad():
            ...
            with torch.autocast(
                device_type='cuda', enabled=self._mixed_precision
            ):
    ...
    def begin_update(self, inputs: ArgsKwargs):
        ...
        def backprop(grads):
            ...
            if found_inf:
                grad_get = (
                    lambda x: x.grad.zero_() if x.grad is not None else x.grad
                )
            else:
                grad_get = lambda x: x.grad

            return convert_recursive(
                lambda x: hasattr(x, 'grad'), grad_get, inputs
            )
    ...
    def use_params(self, params):
        ...
        for k, v in params.items():
            if hasattr(k, 'startswith') and k.startswith(key_prefix):
                state_dict[k.replace(key_prefix, '')] = xp2torch(
    ...
    def from_bytes(self, bytes_data):
        ...
        self.cfg = msg['config']
        self._model = self._deserialize_model(
            self._model, msg['state'], device
        )
