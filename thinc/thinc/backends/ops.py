...
class Ops:
    ...
    @overload
    def unflatten(
        self, X: Ints1d, lengths: Ints1d, pad: int = 0
    ) -> List[Ints1d]: ...

    @overload
    def unflatten(
        self, X: Array2d, lengths: Ints1d, pad: int = 0
    ) -> List2d: ...
    ...
    @overload
    def unflatten(
        self, X: ArrayXd, lengths: Ints1d, pad: int = 0
    ) -> ListXd: ...
    ...
    def pad(  # noqa: F811
        ...
        if len(set(seq.shape[1:] for seq in seqs)) != 1:
            raise ValueError(
                'Cannot pad sequences that differ on other dimensions'
            )
        ...
        output: Array3d = cast(
            Array3d, self.alloc(final_shape, dtype=seqs[0].dtype)
        )
    ...
    def list2padded(self, seqs: List2d) -> Padded:
        ...
        if not seqs:
            ...
            data = self.reshape3(
                seqs[0], seqs[0].shape[0], 1, seqs[0].shape[1]
            )
    ...
    def get_dropout_mask(
        self, shape: Shape, drop: Optional[float]
    ) -> FloatsXd:
    ...
    def alloc3f(
        ...
        *,
        ...
        return cast(
            Floats3d, self.alloc((d0, d1, d2), dtype=dtype, zeros=zeros)
        )
    ...
    def alloc4f(
        ...
        *,
        ...
        return cast(
            Floats4d, self.alloc((d0, d1, d2, d3), dtype=dtype, zeros=zeros)
        )
    ...
    def alloc4i(
        ...
        *,
        ...
        return cast(
            Ints4d, self.alloc((d0, d1, d2, d3), dtype=dtype, zeros=zeros)
        )
    ...
    def reshape4(
        self, array: ArrayXd, d0: int, d1: int, d2: int, d3: int
    ) -> Array4d:
    ...
    def reshape3f(
        self, array: FloatsXd, d0: int, d1: int, d2: int
    ) -> Floats3d:
    ...
    def reshape4i(
        self, array: IntsXd, d0: int, d1: int, d2: int, d3: int
    ) -> Ints4d:
    ...
    def asarray_i(
        self,
        data: Union[IntsXd, Sequence[Any]],
        *,
        dtype: Optional[DTypes] = 'int32',
    ) -> IntsXd:
    ...
    def as_contig(
        self, data: ArrayT, dtype: Optional[DTypes] = None
    ) -> ArrayT:
    ...
    def softmax_sequences(
        ...
        *,
        ...
        summed = self.backprop_reduce_sum(
            self.reduce_sum(new_x, lengths), lengths
        )
        new_x /= summed
        return new_x
    ...
    def backprop_softmax_sequences(
        ...
        dX = Y * dY
        sum_dX = self.backprop_reduce_sum(
            self.reduce_sum(dX, lengths), lengths
        )
        dX -= Y * sum_dX
        return dX
    ...
    def relu_k(
        self, X: FloatsXdT, n: float = 6.0, inplace: bool = False
    ) -> FloatsXdT:
    ...
    def hard_tanh(self, X: FloatsXdT, inplace: bool = False) -> FloatsXdT:
        return self.clipped_linear(
            X, min_val=-1.0, max_val=1.0, inplace=inplace
        )
    ...
    def hard_swish_mobilenet(
        self, X: FloatsXdT, inplace: bool = False
    ) -> FloatsXdT:
        if inplace:
            X *= self.relu_k(X + 3) / 6
            return X
    ...
    def logloss(self, y_true: FloatsT, y_pred: FloatsT) -> float:
        ...
        loss = (y_true * log_yp) + (1 - y_true) * self.xp.log(
            (1 - y_pred) + 1e-8
        )
        return -loss
    ...
    def reduce_sum(self, X: Floats2d, lengths: Ints1d) -> Floats2d:
        ...
        for i, length in enumerate(lengths):
            if length < 0:
                raise ValueError(
                    f'all sequence lengths must be >= 0, got {length}'
                )
    ...
    def reduce_first(
        self, X: Floats2d, lengths: Ints1d
    ...
    def reduce_max(
        self, X: Floats2d, lengths: Ints1d
    ) -> Tuple[Floats2d, Ints2d]:
        Y = self.alloc2f(
            lengths.shape[0], X.shape[1], dtype=X.dtype, zeros=False
        )
    ...
    def backprop_reduce_first(
        ...
        if starts_ends.size == 0:
            return self.alloc2f(
                0, d_firsts.shape[1], dtype=d_firsts.dtype, zeros=True
            )
        ...
        dX = self.alloc2f(
            int(starts_ends[-1]),
            d_firsts.shape[1],
            dtype=d_firsts.dtype,
            zeros=True,
        )
        dX[starts_ends[:-1]] = d_firsts
        return dX

    def backprop_reduce_last(
        self, d_lasts: Floats2d, lasts: Ints1d
    ) -> Floats2d:
        if lasts.size == 0:
            return self.alloc2f(
                0, d_lasts.shape[1], dtype=d_lasts.dtype, zeros=True
            )
        dX = self.alloc2f(
            int(lasts[-1]) + 1,
            d_lasts.shape[1],
    ...
    def backprop_reduce_sum(
        self, d_sums: Floats2d, lengths: Ints1d
    ) -> Floats2d:
        dX = self.alloc2f(
    ...
    def backprop_reduce_mean(
        self, d_means: Floats2d, lengths: Ints1d
    ) -> Floats2d:
        dX = self.alloc2f(
...
def lstm_forward_training(
    ...
    for i in range(depth):
        ...
        for d in range(dirs):
            ...
            layer_params, params_i = _split_weights(
                params, i, nO, nI, params_i
            )
...
def backprop_lstm(
    dY: Floats2d, lengths: Ints1d, params: Floats1d, fwd_state: Tuple
):
    xp = get_array_module(params)
    ...
    params_i = 0
    all_layer_params: List[
        List[Tuple[Tuple[Floats2d, Floats2d, Floats1d], int]]
    ] = []
    for i in range(depth):
    ...
    for i in range(depth):
        ...
        n_inputs = nI if i == 0 else (nO * dirs)
        for d in range(dirs):
            layer_grads, params_i = _split_weights(
                d_params, i, nO, n_inputs, params_i
            )
