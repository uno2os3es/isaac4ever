...
context_ops: ContextVar[Optional[Ops]] = ContextVar(
    'context_ops', default=None
)
...
def use_pytorch_for_gpu_memory() -> None:  # pragma: no cover
    ...
    if 'pytorch' not in pools:
        pools['pytorch'] = cupy.cuda.MemoryPool(
            allocator=cupy_pytorch_allocator
        )
...
def use_tensorflow_for_gpu_memory() -> None:  # pragma: no cover
    ...
    if 'tensorflow' not in pools:
        pools['tensorflow'] = cupy.cuda.MemoryPool(
            allocator=cupy_tensorflow_allocator
        )
...
def get_ops(name: str, **kwargs) -> Ops:
    ...
    ops_by_name = {
        ops_cls.name: ops_cls for ops_cls in registry.ops.get_all().values()
    }  # type: ignore
