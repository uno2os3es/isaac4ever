...
def compile_mmh():
    ...
    return cupy.RawKernel(
        (PWD / '_murmur3.cu').read_text(encoding='utf8'), 'hash_data'
    )
...
backprop_clipped_linear_kernel_double = LazyKernel(
    'backprop_clipped_linear<double>'
)
backprop_clipped_linear_kernel_float = LazyKernel(
    'backprop_clipped_linear<float>'
)
...
def dish(X, *, inplace=False, threads_per_block=128, num_blocks=128):
    ...
    if X.dtype == 'float32':
        dish_kernel_float(
            (num_blocks,), (threads_per_block,), (out, X, X.size)
        )
    else:
        dish_kernel_double(
            (num_blocks,), (threads_per_block,), (out, X, X.size)
...
def gelu(
    X, *, inplace=False, threshold=6.0, threads_per_block=128, num_blocks=128
):
    _is_float_array(X)
...
def mish(
    X, *, inplace=False, threshold=5, threads_per_block=128, num_blocks=128
):
    _is_float_array(X)
...
def swish(
    X, *, inplace=False, threshold=17.0, threads_per_block=128, num_blocks=128
):
    _is_float_array(X)
...
def backprop_seq2col(
    dY, nW, *, lengths=None, threads_per_block=128, num_blocks=128
):
    _is_float_array(dY)
...
def backprop_reduce_sum(
    d_sums, lengths, *, threads_per_block=128, num_blocks=128
):
...
def backprop_reduce_mean(
    d_means, lengths, *, threads_per_block=128, num_blocks=128
):
...
def _check_lengths(lengths, n_elems: int, *, min_length=0):
    assert lengths.dtype == 'int32', (
        'lengths should be encoded as 32-bit integers'
    )
...
def _check_indices(indices, n: int):
    assert indices.dtype == 'int32', (
        'indices should be encoded as 32-bit integers'
    )
