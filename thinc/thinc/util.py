...
DATA_VALIDATION: ContextVar[bool] = ContextVar(
    'DATA_VALIDATION', default=False
)
...
def get_torch_default_device() -> 'torch.device':
    if torch is None:
        raise ValueError(
            'Cannot get default Torch device when Torch is not available.'
        )
...
def require_gpu(gpu_id: int = 0) -> bool:  # pragma: no cover
    ...
    if platform.system() == 'Darwin' and not has_torch_mps:
        if has_torch:
            raise ValueError(
                'Cannot use GPU, installed PyTorch does not support MPS'
            )
...
def to_categorical(
    ...
    *,
    ...
    label_distr = xp.full(
        (n_classes, n_classes), nongold_prob, dtype='float32'
    )
...
def assert_pytorch_installed() -> None:  # pragma: no cover
    ...
    if not has_torch:
        raise ImportError(
            'PyTorch support requires torch: pip install thinc[torch]'
        )


def convert_recursive(
    is_match: Callable[[Any], bool],
    convert_item: Callable[[Any], Any],
    obj: Any,
) -> Any:
    ...
    if is_match(obj):
        ...
        converted = convert_recursive(
            is_match, convert_item, list(obj.items())
        )
        ...
        return [
            convert_recursive(is_match, convert_item, item) for item in obj
        ]
...
def tensorflow2xp(
    tf_tensor: 'tf.Tensor',
    *,
    ops: Optional['Ops'] = None,  # type: ignore
) -> ArrayXd:  # pragma: no cover
