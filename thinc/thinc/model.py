...
context_operators: ContextVar[dict] = ContextVar(
    'context_operators', default={}
)
...
class Model(Generic[InT, OutT]):
    ...
    def grad_names(self) -> Tuple[str, ...]:
        ...
        return tuple(
            [name for name in self.param_names if self.has_grad(name)]
        )
    ...
    def get_dim(self, name: str) -> int:
        ...
        if name not in self._dims:
            raise KeyError(
                f"Cannot get dimension '{name}' for model '{self.name}'"
            )
    ...
    def get_ref(self, name: str) -> 'Model':
        ...
        if name not in self._refs:
            raise KeyError(
                f"Cannot get reference '{name}' for model '{self.name}'."
            )
    ...
    def walk(self, *, order: str = 'bfs') -> Iterable['Model']:
        ...
        if order == 'bfs':
            ...
            raise ValueError(
                'Invalid order, must be one of: bfs, dfs_pre, dfs_post'
            )
    ...
    def get_gradients(
        self,
    ) -> Dict[Tuple[int, str], Tuple[FloatsXd, FloatsXd]]:
    ...
    def _copy(
        ...
        for name in self.param_names:
            params[name] = (
                self.get_param(name) if self.has_param(name) else None
            )
    ...
    def to_dict(self) -> Dict:
        ...
        for node in nodes:
            ...
            for name in node.param_names:
                if node.has_param(name):
                    params[name] = cast(
                        Optional[FloatsXd], node.get_param(name)
                    )
                else:
                    params[name] = None
    ...
    def from_dict(self, msg: Dict) -> 'Model':
        ...
        for i, node in enumerate(nodes):
            ...
            for attr, value in msg['attrs'][i].items():
                ...
                loaded_value = deserialize_attr(
                    default_value, value, attr, node
                )
    ...
    def can_from_disk(
        self, path: Union[Path, str], *, strict: bool = True
    ) -> bool:
    ...
    def can_from_bytes(
        self, bytes_data: bytes, *, strict: bool = True
    ) -> bool:
...
def change_attr_values(
    model: _ModelT, mapping: Dict[str, Dict[str, Any]]
) -> _ModelT:
...
def set_dropout_rate(
    model: _ModelT, drop: float, attrs=['dropout_rate']
) -> _ModelT:
...
def wrap_model_recursive(
    model: Model, wrapper: Callable[[Model], _ModelT]
) -> _ModelT:
