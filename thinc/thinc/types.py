...
class Ragged:
    ...
    def dataXd(self) -> ArrayXd:
        if self.data.size:
            ...
            reshaped = self.data.reshape(
                (self.data.shape[0],) + self.data_shape[1:]
            )
    ...
    def __getitem__(self, index: Union[int, slice, Array1d]) -> 'Ragged':
        ...
        if isinstance(index, int):
            ...
            return Ragged(
                self.data[start:end].reshape(self.data_shape), lengths
            )
        else:
...
class ArgsKwargs:
    ...
    @classmethod
    def from_items(
        cls, items: Sequence[Tuple[Union[int, str], Any]]
    ) -> 'ArgsKwargs':
...
def validate_array(obj, ndim=None, dtype=None):
    ...
    if ndim is not None and obj.ndim != ndim:
        errors.append(
            f'wrong array dimensions (expected {ndim}, got {obj.ndim})'
        )
    if dtype is not None:
        dtype_mapping = {
            'f': ['float32'],
            'i': ['int32', 'int64', 'uint32', 'uint64'],
        }
        expected_types = dtype_mapping.get(dtype, [])
        if obj.dtype not in expected_types:
            expected = '/'.join(expected_types)
            err = (
                f'wrong array data type (expected {expected}, got {obj.dtype})'
            )
            errors.append(err)
    if errors:
